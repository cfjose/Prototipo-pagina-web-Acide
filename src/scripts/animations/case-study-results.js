...
/*
  Adaptado de services-gallery.js — mismo mecanismo de pin + texto
  continuo + barras de progreso. La imagen usa EXACTAMENTE la misma
  transición de siempre (cross-fade + scale, sin tocar).
  ...
  directo la posición según el progreso exacto, así el usuario siente
  que arrastra el texto con su propio gesto de scroll. El texto
  entrante "emerge" literalmente desde la posición real de la barra de
  progreso (medida en pantalla, no un offset inventado como el
  TEXT_RISE=90 que había antes) — [data-case-results-text-stack] tiene
  overflow-hidden para que se corte ahí, más un fundido de opacidad
  angosto solo cerca del borde de recorte.

  La imagen sigue siendo un swap discreto (mismo crossfade de siempre)
  pero ahora se dispara en el PUNTO MEDIO entre dos pasos (Math.round
  del índice virtual), no al arrancar el paso — coincide con el
  momento en que el texto entrante llega al centro de la pantalla.
...
export function initCaseStudyResults() {
  ...
  const textStack = document.querySelector("[data-case-results-text-stack]");
  ...
  /*
    Mismo criterio que ServicesGallery: si las barras no entran sin que
    el stage completo supere el alto del viewport (rompiendo el pin),
    se esconden enteras.

    [data-case-results-text-stack] YA NO tiene overflow-hidden: recortar
    ahí cortaba el texto contra la caja chica del propio bloque (bug
    visto en pantalla — el texto se sentía "encerrado en un frame" en
    vez de salir/entrar de la pantalla real). Ahora el texto viaja libre
    y solo se oculta al salir físicamente del viewport (arriba) o al
    fundirse/desenfocarse (abajo, ver applyContinuousTextPosition).
  ...
  */
  ...
  // Estado inicial: el primer resultado asentado, el resto esperando abajo.
